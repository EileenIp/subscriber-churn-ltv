...
The phases run **in order**, and work **stops at each checkpoint** for review before continuing.

Two kinds of instruction appear below:

- **Build:** implementation work.
- **Decision:** a judgement call to be made explicitly, with the options stated and the reasoning recorded. These are the questions an interviewer will ask, and the answers have to be mine.
...
**Decision:** go or pivot. **Checkpoint −1.**
...
├── NOTES.md             # decision log — written by hand
├── data/
│   ├── raw/             # gitignored
...
**Build:** profile the data and report, without fixing anything yet:
...
**Decision:** what to do about each issue. Drop, impute, or cap — and why. Write the reasoning in `NOTES.md` as you go. This is the "handling ambiguity" criterion straight out of the hiring-manager checklist, and it is the single most common thing that separates a real project from a tutorial clone.
...
**Build:** implement exactly that, in `config.py`. Write a short docstring at the top of `features.py` restating the definition in plain English.
...
**Build:** daily/transaction-level then member-level features from the observation window only. Suggested families:
...
**Decision:** the segment names and what each one means in plain language. "Cluster 3" is not an insight; "lapsing premium-plan subscribers" is.
...
**Build:** draft the README following the SPIDER structure in Appendix A, which is partly filled in already — the sections marked *fill from the run* get completed from actual results, the rest written by hand. Include the live link, a screenshot, and setup instructions someone else could follow.
...
**`NOTES.md` is written by hand.** It is the decision log, in my own words, and it is the evidence of independent work.

**Nothing marked Decision gets settled by default.** Where an answer is needed to keep going, the work stops until it is made deliberately. A decision made to avoid stopping is a decision that can't be defended later.
...
Every session starts by reading, in order: this spec, `NOTES.md`, and the last commit message. Every session ends by committing with a message naming the phase and what changed, and appending a one-line status to the bottom of the spec's checklist.
...
This is the project's planning document and, later, the skeleton of the README. Sections marked **[pre-filled]** are decided already. Sections marked **[fill from the run]** are completed from actual results. Sections marked **[by hand]** must be written by hand — they are the interview answers.
...
**Dead ends and methods that didn't work:** **[This is the section hiring managers actually read, and it gets written by hand. The gaming-to-streaming domain pivot belongs here, in outline — why the original theme didn't survive contact with real data availability.]**
...
- 2026-09-09 — Phase 6 (segmentation and value). `src/segment.py` built: K-Means on behavioural features only (recency/frequency/monetisation/tenure/trajectory — never on `is_churn` or the predicted risk score, which would make "high-risk segment" a tautology), scored population = validation set (genuine held-out LightGBM predictions). k chosen via elbow + silhouette sweep (k=2..8): silhouette peaks at k=3 (0.302), k=7 a close second (0.294); went with k=3 for both statistical support and dashboard interpretability. Segment profiles reviewed unnamed first, then named, grounded directly in the stats: **segment 0 "Drifting Standard Subscribers"** (144,725 / 53.8%, 55.8% churn, moderate tenure, infrequent, silent ~7mo); **segment 2 "Loyal Frequent Renewers"** (116,789 / 43.4%, 17.0% churn, long-tenured, frequent, recently active — the stable core); **segment 1 "High-Value Subscribers Going Dark"** (7,314 / 2.7%, 81.9% churn, ~7x average spend/transaction, long-tenured but infrequent, silent ~8.7mo). Risk-value quadrant (risk threshold = `OPERATING_THRESHOLD` 0.88, value threshold = median `avg_amount_paid_prior` = 149, which happens to be the real modal single-plan price — kept for that reason): **Priority Save** (high risk/high value, 44.8%, 47.6% churn), **Low-Cost Nudge** (high risk/low value, 28.0%, 43.1% churn), **Stable, No Action** (low risk/low value, 15.0%, 9.5% churn), **Quiet Value — Monitor** (low risk/high value, 12.2%, **39.6% churn — deliberately not called "safe"**, since OPERATING_THRESHOLD is tuned for net-benefit-maximizing recall, not for cleanly separating safe from at-risk). Quadrant names and recommendations follow spec Phase 6. 11/11 tests passing (`tests/test_segment.py` added, synthetic fixtures — no dependency on real data being present). Next: Phase 7 (dashboard, presentation, report, website case study).
...
- 2026-09-09 — Phase 7c (written report) built. `deliverables/retention-signal-report.docx` — 5 pages, built with the docx skill (docx-js from scratch, not a template): a standalone executive summary (page 1 — the spec's requirement that reading only page one lets someone act correctly), findings (data & method, honest model comparison, three segments), the risk-value quadrant with per-quadrant recommendations, cost/return under clearly-labeled assumed economics, limitations (marked "for Eileen's review", same drafting discipline as the presentation), and an appendix covering full method — including the three bugs found and fixed during Phase 4/5, named as findings rather than hidden. No template used for either the doc or the docx-js: `python scripts/office/validate.py` clean; visual QA via Word COM automation, since this machine has no LibreOffice (the skill's usual soffice.py path assumes a Linux sandbox and doesn't run on native Windows). Pagination took three iterations to get right: forced page breaks between every major section initially left several pages half-to-fully empty; removing the unnecessary ones and adding `keepNext`/tighter spacing fixed most of it but still left the appendix tail orphaned (a heading alone on one page, four lines split mid-word across the next); the actual fix was combining the appendix's last two short sections into one, which removed the split point entirely and let the whole appendix fit cleanly on a single page. Final: 5 pages, no orphaned headings, no empty pages. Generator kept at `app/build_report.js`. Next: Phase 7d (website case study) — the final Phase 7 deliverable.
- 2026-09-09 — Phase 7d (website case study) built, in `EileenIp.github.io` (a separate repo from this one, at Eileen's explicit direction to work there directly). Replaced the reserved `churn-early-warning-model` gaming placeholder in `data/projects.json` with a full entry (`id: subscriber-churn-retention-2026`) matching the e-commerce case study's exact schema and section depth — summary/problem/inputs/discovery/execution/results, 4 custom SVG visuals (model comparison, renewal-gap curve, Bug 3 before/after, risk-value quadrant), SHAP drivers, and the three real bugs surfaced as a dedicated callout rather than buried in the appendix — built entirely from numbers already established in this spec's session log, none invented. Updated the matching static featured card on the site's homepage (`index.html`) to stay in sync. Caught one real rendering bug before calling it done: the site's results-section renderer only displays a chart whose `id` is exactly `roc-generalization` or `segments` — the quadrant chart was silently dropped under a different id until renamed to `segments`. Verified live via a local server in the browser (not just markup inspection) — modal opens correctly from both the grid card and the `?project=` deep link, all 4 visuals render, full section order matches. Left `links` (notebookRepo/dashboard/caseStudyPage) unset in that entry, since this repo had no GitHub remote at the time — to be added once pushed and Pages is enabled. This closes out Phase 7 (a–d) and the project's build phase. Next: Phase 8 (Eileen's own review pass — trade-off framing, limitations, and final polish — per the spec's judgement-only checkpoint), and pushing this repo to GitHub.
